...
const Deactivate = () => {
    ...
    const {darkMode , customDarkMode , customLightMode , user , setIsAuthenticated , ipAddress} = useContext(AuthContext)

    const deactivate = ()=>{
        const form = {
            userId : user?._id
        }
        axios.post(ipAddress + '/auth/deactivate-acount', form)
            .then(function (response) {
                alert("Acount Deactivated")
    ...
    return !fontsLoaded ? <AppLoading /> : (
        <ScrollView style={{...styles.Mainview,backgroundColor: darkMode ? customDarkMode.backgroundColor : customLightMode.backgroundColor }}>
            <Header endHeading={'About'} headingStyle={{...styles.headingStyle,color:darkMode ? customDarkMode.textColor : customLightMode.textColor}} />


            <View style={{ marginHorizontal: 20, marginTop: 55 }}>
                <Text style={{...styles.title,color:darkMode ? customDarkMode.textColor : customLightMode.textColor}}>{about.title}</Text>
                <Text style={[styles.title, { fontFamily: 'Poppins_700Bold',color:darkMode ? customDarkMode.textColor : customLightMode.textColor }]}>{about.boldText}</Text>
                <Text style={[styles.title, { fontFamily: 'Poppins_400Regular',color:darkMode ? customDarkMode.textColor : customLightMode.textColor }]}>{about.content}</Text>
                <Text style={[styles.title, { fontFamily: 'Poppins_400Regular',color:darkMode ? customDarkMode.textColor : customLightMode.textColor }]}>{about.content2}</Text>
            </View>
            <View style={{ flexDirection: 'row', justifyContent: 'center', marginBottom: 20 }}>
                <YellowBtn onClick={()=> deactivate()} title='Deactivatte' style={{ marginRight: 10 }} />
                <YellowBtn onClick={()=>{
                    navigation.goBack()
                }} title='Decline' style={styles.declineStyle} textStyle={{ color: colors.blue }} />
            </View>
